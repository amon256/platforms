...
public class UserController extends BaseController {
	...
	public String userTree(String account,ModelMap model){
		User existsUser = WebContext.getLoginUser();
		if(account == null){
		...
		if(user == null){
			...
			List<User> users = userService.findList(Query.query(Criteria.where("path").regex("^" + user.getPath()).and("level").gte(user.getLevel()).lte(user.getLevel() + 5)));
